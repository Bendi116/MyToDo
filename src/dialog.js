...
const describeModCurrentInput = document.querySelector(
  "#describe-mod-current-input"
);
const dateModCurrentInput = document.querySelector("#date-mod-current-input");
const priorityModCurrentInput = document.querySelector(
  "#priority-mod-current-input"
);
...
function getProject(e) {
  try {
    submitToDoGlobalBtn.removeEventListener(
      "click",
      globalProject.handleGlobalInput,
      once
    );
  } catch {
    console.log("_");
  ...
  submitToDoGlobalBtn.addEventListener(
    "click",
    globalProject.handleGlobalInput,
    once
  );
  ...
  projectSubmitBtn.addEventListener(
    ...
    (e) => {
      ...
      if (allSelected) {
        ...
        addOptionToProjectSelection(newProject);
      }
...
export function handleToDoGlobalDialog(event, project) {
  ...
  project.addToDoToArray(
    project.createToDo(
      ...
      dateGlobalInput.value ? new Date(dateGlobalInput.value) : null,
      parseInt(priorityGlobalInput.value)
    )
  );
  refreshProject(project);
...
export function showToDoModifyDialog(e) {
  ...
  let project = fromNodeElemenetGetProject(
    e.target.parentElement.parentElement.parentElement.parentElement
  );
  let todo = project.fromNodeElementGetTodo(
